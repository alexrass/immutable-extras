...
describe('List.prototype.mapOn', () => {
  test('empty list', () => {
    ...
    expect(l.mapOn('a')).toEqualImmutable(Immutable.OrderedMap());
  });

  test('passing a string for a list of immutable maps', () => {
    const l = Immutable.fromJS([ { a: 'abc' }, { a: 'def' } ]);
    expect(l.mapOn('a')).toEqualImmutable(Immutable.OrderedMap(Immutable.fromJS({ 'abc': { 'a': 'abc' }, 'def': { 'a': 'def' } })));
  });
  ...
  test('passing a function for list of immutable maps', () => {
    const l = Immutable.fromJS([ { a: 'abc' }, { a: 'def' } ]);
    expect(l.mapOn(i => i.get('a'))).toEqualImmutable(Immutable.OrderedMap(Immutable.fromJS({ 'abc': { 'a': 'abc' }, 'def': { 'a': 'def' } })));
  });
